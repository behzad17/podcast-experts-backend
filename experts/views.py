...
from .models import (
    ExpertProfile, ExpertComment, ExpertReaction, ExpertCategory
)
...
class ExpertProfileCreateView(generics.CreateAPIView):
    ...
    def perform_create(self, serializer):
        if self.request.user.has_created_expert:
            raise PermissionDenied(
                "You have already created an expert profile. Each user is limited to one expert profile."
            )
        ...
        self.request.user.has_created_expert = True
...
class ExpertProfileListView(generics.ListAPIView):
    ...
    search_fields = [
        'specialty', 'user__username', 'bio', 'expertise', 'user__email'
    ]
...
class ExpertProfileViewSet(viewsets.ModelViewSet):
    ...
    def featured(self, request):
        featured_experts = self.get_queryset().filter(is_featured=True)
...
class ExpertReactionViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if reaction_type == 'like':
            if expert.likes.filter(id=self.request.user.id).exists():
                expert.likes.remove(self.request.user)
            else:
                expert.likes.add(self.request.user)
...
class FeaturedExpertsView(APIView):
    def get(self, request):
        # Try to get from cache first
        cache_key = 'featured_experts'

...
class PodcastViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        if self.request.user.has_created_podcast:
            raise PermissionDenied("You have already created a podcast. Each user is limited to one podcast.")
        serializer.save(owner=self.request.user)
        self.request.user.has_created_podcast = True
        self.request.user.save()

    @action(detail=False, methods=['get'])
    def featured(self, request):
        featured_podcasts = self.get_queryset().filter(is_featured=True, is_approved=True)
    ...
    def my_podcasts(self, request):
        podcasts = self.get_queryset().filter(owner=request.user)
        serializer = self.get_serializer(podcasts, many=True)
        return Response(serializer.data)

    @action(detail=False, methods=['get'])
    def stats(self, request):
        user_podcasts = self.get_queryset().filter(owner=request.user)
        total_podcasts = user_podcasts.count()
        total_comments = PodcastComment.objects.filter(
            podcast__owner=request.user
        ).count()
        total_likes = PodcastLike.objects.filter(
            podcast__owner=request.user
        ).count()
...
class FeaturedPodcastsView(APIView):
    def get(self, request):
        # Try to get from cache first
        cache_key = 'featured_podcasts'
